...
- 客户版详细说明书（V6.0.0）：`docs/用户使用说明书-V6.0.0.md`
- 旧版入门手册（V5.0）：`docs/用户使用说明书-V5.0.md`
...
| 套餐 | 价格 | 适用场景 | 核心能力 |
| --- | ---: | --- | --- |
| Plus | ¥1499 | 稳定录入与日常统计 | 手动录入、实时解析、统计导出、规则维护、本地离线，适合稳定业务长期使用 |
| Pro | ¥2999 | 高频业务与自动化 | 在 Plus 基础上提供 OCR 识别、语音录入、本地AI修正、微信自动监听与持续更新，适合追求效率和规模化运营 |

### 套餐规则
- 试用期默认开放 Plus 能力（到期后需授权）
- Plus 核心价值：`手动录入`、`实时解析`、`统计导出`、`规则维护`、`一次买断`
- Pro 专属价值：`OCR 智能识别引擎`、`语音录入`、`本地AI修正`、`微信自动监听去重`、`持续更新`
